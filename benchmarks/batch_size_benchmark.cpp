                             ...
                             bool verbose = true)
{
    ...
    config.batchSize = writerBatchSize; // number of entries a single writer thread can dequeue at once at most
    ...
    if (verbose)
    {
        std::cout << "Logging system started with writer batch size: " << writerBatchSize << std::endl;
...
void runBatchSizeComparison(const std::vector<int> &batchSizes,
                            ...
                            int entriesPerProducer, int numSpecificFiles, int producerBatchSize)
{
    std::cout << "\n============== WRITER BATCH SIZE BENCHMARK ==============" << std::endl;
    std::cout << "Testing performance with different writer batch sizes" << std::endl;
    ...
    {
        double relativePerf = throughputs[i] / throughputs[0]; // Relative to smallest batch size
